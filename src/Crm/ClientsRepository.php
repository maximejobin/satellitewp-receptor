...
final class ClientsRepository
{
    ...
    public function productsLastSyncedAt(): ?string
    {
        $value = $this->pdo->query('SELECT MAX(date_sync) FROM swp_products')->fetchColumn();
    ...
    public function subscriptionsForClient(int $clientId): array
    {
        $stmt = $this->pdo->prepare(<<<'SQL'
            ...
            ORDER BY
                CASE
                    WHEN mp.auto_id IS NOT NULL THEN 0
                    WHEN l.auto_id IS NOT NULL THEN 2
                    ELSE 1
                END,
                CASE WHEN l.auto_id IS NOT NULL THEN p.name END ASC,
    ...
    public function listWebsites(
        ...
        if ($excludeTags !== null && $excludeTags !== []) {
            $placeholders = [];
        ...
        $clientsByWebsite = $this->clientsByWebsiteIds($ids);

        foreach ($websites as &$website) {
            $website['tags']    = $tagsByWebsite[(int) $website['id']] ?? [];
            $website['clients'] = $clientsByWebsite[(int) $website['id']] ?? [];
        }
     ...
     */
    private function clientsByWebsiteIds(array $websiteIds): array
    {
        if ($websiteIds === []) {
            return [];
        }
        ...
        $placeholders = implode(',', array_fill(0, count($websiteIds), '?'));
        $stmt         = $this->pdo->prepare(<<<SQL
            SELECT DISTINCT sw.website_id AS __website_id, c.*
            FROM swp_subscriptions_websites sw
            JOIN swp_subscriptions s ON s.id = sw.subscription_id
            JOIN swp_clients c ON c.id = s.client_id
            WHERE sw.website_id IN ({$placeholders})
            ORDER BY c.company, c.last_name, c.first_name
            SQL);
        $stmt->execute($websiteIds);
        ...
        $result = [];
        foreach ($stmt->fetchAll() as $row) {
            $websiteId = (int) $row['__website_id'];
            unset($row['__website_id']);
            $result[$websiteId][] = $row;
        }
